...
# This method object works as a function to get randoms acording to the rejection w/o repetition algorithm.
rejection = MyRejectionFunction()
    ...
    for n in range(N):
        ...
        if gaussian == True:
            if N != 1:
                pos = int(N*ran())
                x = prevx[0:pos] + \
                    [random.gauss(prevx[pos], delta)]+prevx[pos+1:N]
            elif N == 1:
